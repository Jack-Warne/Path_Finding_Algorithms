...
# this is where the algorithm will be coded

def algorithm(draw, grid, start, end):
    count = 0 
    open_set = PriorityQueue()
    open_set.put((0, count, start)) # add start node with its fscore into the queue, count is added to break ties if two cubes have the same fscore
    came_from = {} # what node did it come from to keep track of path

    # setting gscores and hscores
    g_score = {node: float("inf") for row in grid for node in row} # setting gscores which are unkown therefore infinite
    g_score[start] = 0
    
    f_score = {node: float("inf") for row in grid for node in row}
    f_score[start] = h(start.get_pos(), end.get_pos()) # hscore is heuristic is to estimate the distance between the start and end nodes to make sure when we reach the node we don't assume it is the best path
    ...
    while not open_set.empty(): 
        ...
        for neighbour in current.neighbours:
            ...
            if temp_g_score < g_score[neighbour]: # if gscore is better then update values to make a better path
                ...
                if not neighbour.is_closed():
                    ...
                    neighbour.OPEN() # show we've already considered the neighbour
        draw()
        
        if current != start:
            current.CLOSE() # if the node we just considered isn't start close it because we've considered it
    return False # returns if we don't find a path
